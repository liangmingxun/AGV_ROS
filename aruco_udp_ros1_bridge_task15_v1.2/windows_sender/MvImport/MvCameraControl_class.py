...
_mvs_dll = next((item for item in _mvs_candidates if item and Path(item).is_file()), None)
if _mvs_dll is None:
    raise OSError("MvCameraControl.dll not found; install MVS or set MVS_DLL_PATH")
MvCamCtrldll = WinDLL(_mvs_dll)
