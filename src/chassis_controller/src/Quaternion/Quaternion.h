...
inline void Quaternion_GetAngularVelocity_Body(Eigen::Quaternionf q, Eigen::Vector4f dq, Eigen::Vector3f& omega_body_out) {
    float q_[4] = {q.w(), q.x(), q.y(), q.z()};
    float dq_[4] = {dq[0], dq[1], dq[2], dq[3]};
    Quaternion_GetAngularVelocity_Body(q_, dq_, omega_body_out.data());
}
