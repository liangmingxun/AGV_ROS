...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "usage: $0 --candidate Y0_gamma0p12_T3p5|Y1_gamma0p10_T3p5|Y2_gamma0p08_T3p5 --method M1|M1b|pair [--output-root DIR] [--ros-port PORT]"
...
if [[ -z "$output_root" ]]; then
  output_root="${workspace}/experiment_data/exp2c_v5_yaw_effectiveness_exploration/EFFECT_EXPLORATION_ONLY_NOT_FORMAL_PAPER_EVIDENCE_$(date +%Y%m%d_%H%M%S)"
fi

[[ "$candidate" == Y0_gamma0p12_T3p5 || "$candidate" == Y1_gamma0p10_T3p5 || "$candidate" == Y2_gamma0p08_T3p5 ]] || { echo "Invalid exploration candidate" >&2; exit 2; }
...
mkdir -p "$output_root"
python3 "${workspace}/src/multi_agv_analysis/scripts/analyze_exp2c_v5_yaw_effectiveness.py" --prepare "$output_root"
...
run_one() {
  ...
  roslaunch multi_agv_bringup formal_evaluation_window.launch \
    ...
    method_id:="$method_id" experiment_id:=exp2c_v5_yaw_effectiveness_exploration \
  ...
  roslaunch multi_agv_bringup experiment.launch \
    ...
    run_id:="$method_id" experiment_id:=exp2c_v5_yaw_effectiveness_exploration \
  ...
  if (( SECONDS >= deadline )); then
    echo "EXP2C-V5 TASK_TIMEOUT: incomplete run retained for full failure report" >&2
  fi
  sleep 1
  ...
  if ! rosrun multi_agv_analysis process_experiment_run.py \
    ...
    echo "EXP2C-V5: postprocess invalid; retain original bag/CSV/validation for classification" >&2
  fi
  python3 "${workspace}/src/multi_agv_analysis/scripts/analyze_exp2c_v5_yaw_effectiveness.py" \
