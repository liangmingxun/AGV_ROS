...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --baseline-log) baseline_log="$2"; shift 2 ;;
    --output-root) [[ $# -ge 2 ]] || exit 2; output_root="$2"; shift 2 ;;
    --ros-port) [[ $# -ge 2 ]] || exit 2; ros_port="$2"; shift 2 ;;
    -h|--help)
      echo "usage: $0 --candidate E0..E4 --method M1|M1b|pair [--output-root DIR] [--ros-port PORT]"
      echo "EFFECT_EXPLORATION_ONLY / NOT_FORMAL_PAPER_EVIDENCE; E0 can use --baseline and --baseline-log."
      exit 0 ;;
...
if [[ -z "$output_root" ]]; then
  output_root="${workspace}/experiment_data/exp2c_v4_effect_exploration/EFFECT_EXPLORATION_ONLY_NOT_FORMAL_PAPER_EVIDENCE_$(date +%Y%m%d_%H%M%S)"
fi

[[ "$candidate" =~ ^E[0-4]$ ]] || { echo "Invalid exploration candidate" >&2; exit 2; }
...
mkdir -p "$output_root"
python3 "${workspace}/src/multi_agv_analysis/scripts/analyze_exp2c_v4_effect_exploration.py" --prepare "$output_root"
...
run_one() {
  ...
  roslaunch multi_agv_bringup formal_evaluation_window.launch \
    ...
    method_id:="$method_id" experiment_id:=exp2c_v4_effect_exploration \
  ...
  roslaunch multi_agv_bringup experiment.launch \
    ...
    run_id:="$method_id" experiment_id:=exp2c_v4_effect_exploration \
  ...
  while (( SECONDS < deadline )); do
    ...
    if grep -Eq "Runtime rigid-fit hard gate rejected|safety abort latched|held fail-zero:" "${log_dir}/algorithm.log"; then
  ...
  python3 "${workspace}/src/multi_agv_analysis/scripts/analyze_exp2c_v4_effect_exploration.py" \
...
if [[ "$method" == M1 && -n "$baseline" && -n "$baseline_log" ]]; then
  python3 "${workspace}/src/multi_agv_analysis/scripts/analyze_exp2c_v4_effect_exploration.py" \
    --root "$output_root" --candidate "$candidate" --baseline "$baseline" --baseline-log "$baseline_log"
fi
