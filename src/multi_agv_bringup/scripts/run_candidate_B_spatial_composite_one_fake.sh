...
awk -v value="$target" 'BEGIN {exit !(value>=3.0 && value<=5.178229715025710)}' || {
  echo "ERROR: --target must be within [3.0,5.178229715025710]" >&2; exit 2; }
...
validation_config="$output_root/configs/validation_candidate_b_spatial.yaml"
...
roslaunch multi_agv_bringup experiment.launch arming_authorized:=true \
  ...
  evaluation_target:="$target" nominal_common_velocity:=0.10 \
...
deadline=$((SECONDS+100)); reached=false
while (( SECONDS < deadline )); do
  ...
  if [[ -n "$progress" ]] && awk -v value="$progress" -v goal="$target" \
      'BEGIN {exit !(value>=goal)}'; then reached=true; break; fi
