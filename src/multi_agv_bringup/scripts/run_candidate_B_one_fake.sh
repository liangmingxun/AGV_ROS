...
output_root=""; ros_port=11690; method=""; stage=Triad01
while [[ $# -gt 0 ]]; do
  case "$1" in
...
python3 src/multi_agv_analysis/scripts/analyze_candidate_B_fake.py \
...
validation_config="$output_root/configs/validation_candidate_b.yaml"
...
roslaunch multi_agv_bringup formal_evaluation_window.launch \
  ...
  experiment_id:=candidate_B_effectiveness_fake_validation \
...
while (( SECONDS < deadline )); do
  ...
  if [[ -n "$progress" ]] && awk -v v="$progress" 'BEGIN {exit !(v>=5.178229715025710)}'; then
    reached=true; break
  fi
...
[[ "$reached" == true ]] || echo "INVALID_RUN: target not reached" >&2
sleep 1
...
echo "CANDIDATE_B_RUN_DIR=$output_root/run"
