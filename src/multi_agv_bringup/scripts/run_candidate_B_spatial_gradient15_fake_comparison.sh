...
[[ -n "$output_root" ]] || output_root="$workspace/experiment_data/candidate_B_spatial_gradient15_fake/CANDIDATE_B_SPATIAL_GRADIENT15_$(date +%Y%m%d_%H%M%S)"
...
printf '%s\n' 'EXPLORATORY_FAKE_VALIDATION / NOT_FORMAL_PAPER_EVIDENCE / Candidate B v2 gradient15 / no serial' >"$output_root/SCOPE.txt"
...
cp "$output_root/candidate-B-spatial-gradient15-rho0p85-av0p020-fake-results.md" \
  docs/test-protocols/candidate-B-spatial-gradient15-rho0p85-av0p020-fake-results.md
