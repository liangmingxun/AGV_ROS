...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "usage: $0 --candidate S1_gamma0p20_down0p6_hold2p5_up0p6_R10 --method M1|M1b|M2b [--output-root DIR] [--ros-port PORT]"
      echo "REPEAT_VALIDATION / NOT_FORMAL_PAPER_EVIDENCE; one fresh M1, M1b, or complete M2b run."
      exit 0 ;;
...
if [[ -z "$output_root" ]]; then
  output_root="${workspace}/experiment_data/exp2c_v5b_s1_three_method_repeat_validation/REPEAT_VALIDATION_NOT_FORMAL_PAPER_EVIDENCE_$(date +%Y%m%d_%H%M%S)"
fi
# roslaunch changes its working directory to ~/.ros.  Normalize caller-supplied
# relative paths before passing generated configuration files to ROS nodes.
if [[ "$output_root" != /* ]]; then
  output_root="${workspace}/${output_root#./}"
fi

[[ "$candidate" == S1_gamma0p20_down0p6_hold2p5_up0p6_R10 ]] || { echo "Invalid exploration candidate" >&2; exit 2; }
[[ "$method" == M1 || "$method" == M1b || "$method" == M2b ]] || { echo "Invalid exploration method" >&2; exit 2; }
cd "$workspace"
...
mkdir -p "$output_root"
python3 "${workspace}/src/multi_agv_analysis/scripts/analyze_exp2c_v5b_three_method_repeat.py" --prepare "$output_root" --method "$method"
echo "REPEAT_VALIDATION / NOT_FORMAL_PAPER_EVIDENCE"
...
run_one() {
  ...
  elif [[ "$method" == M2b ]]; then
    method_id=M2b_M2b
    upper_config="${output_root}/configs/${candidate}_M2b.yaml"
    authorization_config="${output_root}/configs/${candidate}_M2b_authorization.yaml"
  ...
  echo "REPEAT_VALIDATION NOT_FORMAL_PAPER_EVIDENCE FAKE: stage=${stage} method=${method_id} candidate=${candidate}"
  ...
  roslaunch multi_agv_bringup formal_evaluation_window.launch \
    ...
    method_id:="$method_id" experiment_id:=exp2c_v5b_s1_three_method_repeat_validation \
  ...
  roslaunch multi_agv_bringup experiment.launch \
    ...
    run_id:=run experiment_id:=exp2c_v5b_s1_three_method_repeat_validation \
  ...
  python3 "${workspace}/src/multi_agv_analysis/scripts/analyze_exp2c_v5b_three_method_repeat.py" \
    --method "$method" --check-run "$run_dir" --check-log "${log_dir}/algorithm.log"

}

# One invocation owns a fresh private master and exactly one method run.
run_one S1 "$method" "$candidate"
echo "REPEAT_VALIDATION_RUN_DIR=${output_root}/run"
