...
FormalExecutionGateResult evaluateFormalSerialM1R1Gate(
  ...
  if (!input.m1_r1_selected && !input.m1b_r1_selected && !input.m2a_r1_selected &&
      !input.m2b_m2b_selected) {
    return {
        false,
        "formal serial entry permits M1+R1, M1b+R1, M2a+R1 or M2b+M2b only"};
  }
