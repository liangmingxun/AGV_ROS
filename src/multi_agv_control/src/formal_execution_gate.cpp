...
FormalExecutionGateResult evaluateFormalSerialM1R1Gate(
  ...
  if (!input.m1_r1_selected && !input.m1_paper_nm_selected &&
      !input.m1_pd_penalty_selected && !input.m1b_r1_selected &&
      !input.m2a_r1_selected && !input.m2b_m2b_selected) {
    return {
        false,
        "formal serial entry permits M1+R1, M1+PaperNM, M1+PDPenalty, "
        "M1b+R1, M2a+R1 or M2b+M2b only"};
  }
  if (input.m1b_r1_selected && !input.m1b_hardware_authorized) {
    return {false, "M1b serial hardware authorization is disabled"};
  }
