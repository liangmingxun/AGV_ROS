...
V2 = V4.V2
IDENTITY = 'exp2c_v5_yaw_effectiveness_exploration'
MARKERS = ['EFFECT_EXPLORATION_ONLY', 'NOT_FORMAL_PAPER_EVIDENCE']
CANDIDATES = {'Y0_gamma0p12_T3p5': (.12,3.5), 'Y1_gamma0p10_T3p5': (.10,3.5), 'Y2_gamma0p08_T3p5': (.08,3.5)}
...
def prepare(root):
    ...
    save(root/'EXPLORATION_SCOPE.json', {'experiment_id': IDENTITY, 'markers': MARKERS,
         ...
         'success_rule': 'active risk/reference mechanism plus wheel-risk benefit and >=3% geometry IAE improvement'})
    for candidate, (amplitude, duration) in CANDIDATES.items():
        for method in ('M1', 'M1b'):
            ...
            runtime['yaw_effectiveness_v5'] = {'enabled': True, 'trigger_progress': 2., 'gamma_min': amplitude, 'duration': duration}
            ...
            auth['yaw_effectiveness_v5'] = {'hardware_execution_authorized': False}
    ...
    for key in ('topics','required_topics'):
        registry[key].append('/multi_agv/yaw_effectiveness_state')
...
def extras(rows):
    ...
    result['physical_speed_limiter_first_input_output']={
        f'agv{i}_{side}': {'input_degraded_or_raw':V2.num(limiter_rows[0],
             'yaw_effectiveness_'+('left_degraded' if side=='left' else 'right_degraded')) if i==2 else V2.num(limiter_rows[0],f'agv{i}_wheel_{side}_pre_limit'),
...
def exposure(rows, values, threshold=None):
    ...
    return {'peak':max(values), 'RMS':math.sqrt(V4.weighted_mean([v*v for v in values],dt)),
...
def quality_metrics(rows):
    ...
    for i in (1,2,3):
        for key in ('lateral','heading','longitudinal'):
            result[f'agv{i}_{key}']=exposure(rows,[V2.num(r,f'agv{i}_tracker_{key}_error') for r in rows])
...
def measured_rigid_quality(run, trigger, task_rows, total):
    ...
    for name,stats in result.items():
        ...
        stats['first_exceedance_context']={k:V2.num(context,k) for k in (
            'yaw_effectiveness_gamma','yaw_effectiveness_left_raw','yaw_effectiveness_right_raw',
            'common_velocity_reference','agv2_tracker_lateral_error','agv2_tracker_heading_error',
            'agv2_robust_margin','yaw_effectiveness_causal_wheel_margin')} if context else None
    return result
...
def whole_recorded_execution(all_rows, diagnostic_rows):
    ...
    trigger=next(V2.num(r,'yaw_effectiveness_trigger_wall_time') for r in diagnostic_rows if V2.flag(r,'yaw_effectiveness_triggered'))
    result={'raw_samples':len(samples),'missing_raw_samples':len(all_rows)-len(samples),
            ...
            'v5_native_diagnostic_start_delay_from_recording_seconds':V2.num(diagnostic_rows[0],'yaw_effectiveness_source_stamp')-V2.num(all_rows[0],'stamp'),
...
def analyze(run, candidate, log_path, historical=False):
    ...
    if not historical and runtime['yaw_effectiveness_v5'].get('enabled') is not True:
    ...
    pulse_config = runtime['yaw_effectiveness_v5']
    if abs(pulse_config['gamma_min']-amplitude)>1e-12 or abs(pulse_config['duration']-total)>1e-12 or pulse_config['trigger_progress'] != 2.:
        raise ValueError('incorrect exploratory pulse')
    with (run/'converted/aligned_samples.csv').open(newline='') as stream:
        all_rows = list(csv.DictReader(stream))
    rows=[]; seen=set()
    for row in all_rows:
        if not V2.flag(row,'algorithm_valid') or not V2.flag(row,'yaw_effectiveness_state_available'): continue
        row = dict(row)
        for key,value in list(row.items()):
            if key.startswith('yaw_effectiveness_'): row[key.replace('yaw_effectiveness_','transient_yaw_',1)] = value
        row['transient_yaw_left_disturbed']=row['yaw_effectiveness_left_degraded']
        row['transient_yaw_right_disturbed']=row['yaw_effectiveness_right_degraded']
        wall=V2.num(row,'yaw_effectiveness_wall_time')
    ...
    for row in rows:
        ...
        for key,field in (('heading','heading_error'),('lateral','lateral_error'),('longitudinal','longitudinal_error')):
            row['errors'][key]=V2.num(row,'agv2_tracker_'+field)
    ...
    waveform=all(abs(V2.num(r,'yaw_effectiveness_gamma')-(1.-(1.-amplitude)*V2.num(r,'yaw_effectiveness_envelope')))<1e-12
                 and amplitude-1e-12<=V2.num(r,'yaw_effectiveness_gamma')<=1.+1e-12
                 and abs(V2.num(r,'yaw_effectiveness_mean_longitudinal_delta'))<1e-12
                 and (r['relative_wall_time']<total or V2.num(r,'yaw_effectiveness_gamma')==1.) for r in rows)
...
def comparison(m1,m1b):
    ...
    found=usable and all(mechanism.values()) and any(clear.values()) and any(value>=.03 for value in geometry.values())
    return {'experiment_id':IDENTITY,'markers':MARKERS,'candidate':m1['candidate'],
            'status':'YAW_EFFECTIVENESS_WORKING_POINT_FOUND' if found else 'EFFECT_INSUFFICIENT' if usable else 'CANDIDATE_HARD_STOP',
...
def main():
    ...
    (out/'effect_comparison.md').write_text('\n'.join(lines)+'\n')
    print(result['status'])
    return 0 if result['status']=='YAW_EFFECTIVENESS_WORKING_POINT_FOUND' else 10 if result['status']=='EFFECT_INSUFFICIENT' else 20
