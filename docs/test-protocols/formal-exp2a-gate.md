...
`0.15→0.102→0.15 m/s`公共实车降额锁；本次授权不冒充举升预检结果，后续
运行清单仍应如实保留授权依据。正式统计审批没有因此自动开启。

当前圆形降额入口统一使用 R=0.7 m 顺时针平滑进出路径和 2.5 s 缓启动。
Robot2 在等效载荷实际进度 `1.50 m` 开始用 1.00 s 从 `0.15 m/s`
降到 `0.102 m/s`，在 `3.50 m` 开始用 1.00 s 恢复。该 `0.68` 比例与论文
仿真的能力变化幅度一致，也避免首轮 `0.40` 降额造成的低速执行不连续。两个触发点均位于
