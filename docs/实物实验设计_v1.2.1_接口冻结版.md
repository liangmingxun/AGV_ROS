...
$a_i=-0.01783\ \mathrm m,b_{s,i}=0$。先从期望支撑点路径 $p_i(s)$
...
由于当前支撑点位于驱动轮轴后方，启动时从路径终点航向边界反向积分
该方程，避免后置控制点的前向不稳定零动态。然后：
...
- 当前 CAD/实测 `base_link -> support_link` 为
  `x=-0.01783 m, y=0, yaw=0`。
